...
int main(){
    ...
    Position head3 = NULL;
    head3 = (Position)malloc(sizeof(Polynomial));
    if(head3 == NULL){
        printf("Memory allocation error...\n");
        return -1;
    }
    ...
    InsertSorted(head1, 3, 4);
    InsertSorted(head1, 2, 5);

    Display(head1->next);

    InsertSorted(head2, 5, 3);
    InsertSorted(head2, 4, 5);

    Display(head2->next);

    InsertSorted(head3, 0, 1);
    InsertSorted(head3, 1, 1);
    Display(head3->next);

    Position sum = AddPolynomials(head1, head3);
    result = MultiplyPolynomials(head1, head3);
    Display(result);
